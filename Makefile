...
PKG := github.com/natemarks/netsumm
COMMIT := $(shell git rev-parse HEAD)
PKG_LIST := $(shell go list ${PKG}/... | grep -v /vendor/)
GO_FILES := $(shell find . -name '*.go' | grep -v /vendor/)
CDIR = $(shell pwd)
EXECUTABLES := netsumm netsimple
GOOS := linux
GOARCH := amd64
	...
	@for o in $(GOOS); do \
	  for a in $(GOARCH); do \
        ...
        go build  -v -o build/$(COMMIT)/$${o}/$${a}/$@ \
				-ldflags="-X github.com/natemarks/netsumm/version.Version=${COMMIT}" ${PKG}/cmd/$@; \
	  done \
    done ; \
...
release: git-status build
	...
	@for o in $(GOOS); do \
	  for a in $(GOARCH); do \
        tar -C ./build/$(COMMIT)/$${o}/$${a} -czvf release/$(COMMIT)/netsumm_$(COMMIT)_$${o}_$${a}.tar.gz . ; \
	  done \
    done ; \
